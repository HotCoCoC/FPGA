...
def mask_depth(image,th,throld = 15000):
    global nr
    global nc
   # nr = image.shape[0]
   # nc = image.shape[1]
...
while True:
    ...
    if ret1 != True:
        break
    cv2.resize(frame,(2560,480),interpolation=cv2.INTER_LINEAR)  
    dsize = (1280, 480)
    ...
    stereo = cv2.StereoBM_create(numDisparities = 32*num, 
                                  blockSize = 31)
...
#    disp1 = disp.copy()
#    mask_depth(disp,disp1)
    ret, binary = cv2.threshold(disp,127,255,cv2.THRESH_BINARY) 
#    cv2.imshow("left", img1_rectified)
    cv2.imshow("right", img2_rectified)
    cv2.imshow("depth", disp)
    cv2.imshow("depth1", binary)
